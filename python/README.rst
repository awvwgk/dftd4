...
This project support poetry as an easy way to build the Python API.
...
- Python 3.6 or newer with the `CFFI <https://cffi.readthedocs.io/>`_ and `pkgconfig <https://pypi.org/project/pkgconfig/>`_ packages installed

You can let poetry install the project dependencies for you, include at least the *build* extra, which is needed to build the extension module but not to run dftd4

.. code:: sh

   poetry install --no-root -E build

You can let poetry handle the complete install by removing the (``--no-root``) option, if you prefer to work in poetry.
...
Now you can install the Python API outside of poetry with
